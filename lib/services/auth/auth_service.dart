...
class AuthService {
  ...
  /// URL, с которым страница была открыта (веб): main() снимает его ДО
  /// [init] — Supabase вычищает auth-параметры (`?code=`, `#error=…`) из
  /// адресной строки, а по ним приложение показывает человеку, что вход по
  /// ссылке не удался (влад. 2026-07-19: «белый экран, не залогинилось» —
  /// раньше ошибка тонула молча).
  static Uri? launchUri;

  /// Ошибка [init] (например, PKCE-обмен по ссылке из ДРУГОГО браузера):
  /// main() ловит её, чтобы приложение не умирало белым экраном, и приложение
  /// после первого кадра показывает пояснение.
